...
| PostgreSQL + docker-compose | БД поднимается, приложение подключается                    |   2    |
| Flyway                      | Миграции создают полную схему, `ddl-auto=validate`         |   2    |
...
| CRUD: Restaurant            | Полный CRUD работает через API                             |   2    |
| CRUD: Order                 | Создание и чтение заказов с блюдами                        |   2    |
| @EntityGraph / JOIN FETCH   | N+1 отсутствует при чтении связанных данных                |   2    |
| Качество решения            | Архитектура, маппинг Entity↔DTO, чистота кода              |   1    |
| **Итого**                   |                                                            | **15** |
